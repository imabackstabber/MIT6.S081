...
// for lab3 copyin_new
// copy every each pte in user pgtbl to kernel pgtbl
void
u2kvmcopy(pagetable_t old, pagetable_t new, uint64 oldsz, uint64 sz)
{
  pte_t *pte,*new_pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = oldsz;i < sz;i += PGSIZE){
    ...
    if((new_pte = walk(new,i,1)) == 0){
      panic("u2kvmcopy: get new pte error");
      goto err;
    }
